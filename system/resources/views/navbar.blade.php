...
a {
  ...
  <!--Navbar -->
    <header class="mb-5">
    <nav style="background: hsl(var(--header-color)/.75)" class="fixed top-0 z-40 w-full border-b border-t bg-gray-600/75 border-transparent backdrop-blur " data-headlessui-state="open" data-aos="fade-down">
...
<!-- Show Daftar / Masuk -->
    <div class="flex flex-1 items-center sm:items-center sm:justify-start ml-12 gap-4">
        <div class="flex flex-shrink-0 items-center">
            <a class="relative w-auto" href="{{ route('home') }}">
                <img alt="logo" srcset="{{ !$config ? '' : $config->web_logo }}" src="" width="50" height="50" decoding="async" data-nimg="1" class="block h-12 w-auto" loading="lazy" style="color: transparent;" />
            </a>
        </div>
...
<div class="flex gap-2 pr-0 lg:pr-2"></div>
    ...
    @guest
      <button class="inline-flex d-block d-lg-block justify-center rounded-md py-2 text-xs font-medium hover:bg-opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75" id="headlessui-menu-button-:R1qd6:" type="button" aria-haspopup="menu" aria-expanded="false" data-headlessui-state="closed" aria-controls="headlessui-menu-items-:r0:">
            <div class="flex gap-2 p-2 lg:pr-2 btn-light w-full flex justify-center rounded-lg py-2 text-sm font-medium md:inline-flex transition duration-300 css-mxl5am ">
       ...
       <div class="absolute right-0 mt-2 w-72 origin-top-right divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-non css-18exuzb transform opacity-0 scale-95 hidden" aria-labelledby="headlessui-menu-button-:R1qd6:" id="headlessui-menu-items-:r0:" role="menu" tabindex="0" data-headlessui-state="closed">
            ...
            <div class="px-1 py-1" role="none" id="headlessui-menu-item-:r1:" role="menuitem" tabindex="-1" data-headlessui-state="1">
                          ...
                          @if(Auth::user()->role == "Admin")
                             <a class="group flex w-full items-center rounded-md px-2 py-2 text-sm transition duration btn-patih" id="headlessui-menu-item-:rf:" role="menuitem" tabindex="-1" data-headlessui-state="" href="{{ route('dashboard') }}"><i class="fa-sharp fa-solid fa-screwdriver-wrench"></i>&nbsp;MyAdmin</a>
                          @endif
                          @endauth
                      	  @auth
                      	 @if(Auth::user()->role !== "Admin")
                <a class="group flex w-full items-center rounded-md px-2 py-2 text-sm transition duration hover:bg-gray-50" id="headlessui-menu-item-:rf:" role="menuitem" tabindex="-1" data-headlessui-state="" href="{{url('/user/dashboard')}}" ><i class="fas fa-home"></i>&nbsp;Dashboard</a>
                  @endif
                <a class="group flex w-full items-center rounded-md px-2 py-2 text-sm transition duration hover:bg-murky-700" id="headlessui-menu-item-:rg:" role="menuitem" tabindex="-1" data-headlessui-state="" href="/deposit">
                   <i class="fas fa-wallet"></i>&nbsp;IDR  {{ number_format(Auth::user()->balance, 0, ',', '.') }}
                </a>
                ...
                <div class="px-1 py-1" role="none"><style>
                    ...
                    <div id="headlessui-menu-item-:r1:" role="menuitem" tabindex="-1" data-headlessui-state="1">
                        <div class="text-center mt-4">
                            <img alt="mbgs-logo-" srcset="{{ !$config ? '' : $config->web_logo }}" src="{{ !$config ? '' : $config->web_logo }}" width="150" height="120" decoding="async" data-nimg="1" class="mx-auto" loading="lazy" style="color: transparent;"/>
                            <p class="text-md font-normal mt-2">Selamat datang di <strong>{{ config('app.name') }}</strong></p>
                        </div>
                            <div class="flex w-full p-3">
                             
                              @guest
                                <a class="btn-voucher btn btn-light btn-primary w-full flex justify-center rounded-l-full py-2 text-sm font-medium md:inline-flex transition duration-300 css-1wpp9sf" href="{{ route('login') }}">Masuk</a>
                                @endguest
                                <a class="btn-voucher btn-light w-full flex justify-center rounded-r-full py-2 text-sm font-medium md:inline-flex transition duration-300 css-mxl5am" href="{{ route('register') }}">Daftar</a>
                                </div>
                            </div>
                        </div>
                    </div>
...
<div id="searchContainer"  class="col-lg-11 mx-auto w-full max-w-7xl p-4 sm:p-8">
  <div class="flex w-full items-center md:mx-auto lg:mx-0 xl:px-0">
    <form class="relative w-full">
      ...
      <input id="searchInput" class="block w-full rounded-full border-none py-2 pl-5 gap-3 pr-3 text-sm text-dark focus:outline-none focus:ring-1 fsm:text-sm md:py-3 ms-" placeholder="Cari ..." type="search" autocomplete="off" />
    </form>
  </div>
   <ul class="p-2 px-3 bg-white rounded-4 mt-3 text-dark d-none" id="live_search_ul" style="list-style: none; max-height: 600px; overflow-x:scroll"></ul>
</div>
</div>
<style>
...
.offcanvas-backdrop.show {
  ...
  backdrop-filter: blur(8px);
    ...
    <div class="flex shrink-0 items-center px-4">
        <div class="offcanvas offcanvas-start offcanvas-backdrop.show bg-gray-700" style="background: linear-gradient(to right, hsl(var(--bg-gradient-from)) 0%, hsl(var(--bg-gradient-via)) 50%, hsl(var(--bg-gradient-to))) 100% !important;" tabindex="-1" id="offcanvasDarkNavbar" aria-labelledby="offcanvasDarkNavbarLabel"  visibility: visible;" aria-modal="true" role="dialog">
            <div class="offcanvas-header" >
                <h5 class="offcanvas-title" id="offcanvasDarkNavbarLabel">
                    <img src="" alt="" width="100">
                </h5>
                <button type="button" class="rounded-full text-gray-400 hover:text-btnPurple focus:outline-none focus:ring-2 focus:ring-white" data-bs-dismiss="offcanvas" aria-label="Close">
            ...
            <a class="relative h-[5.7rem] w-50" href="{{ route('home') }}">
            <img alt="logo" srcset="{{ !$config ? '' : $config->web_logo }}" src="{{ !$config ? '' : $config->web_logo }}" decoding="async" data-nimg="fill" loading="lazy" style="position: absolute;   inset: 0px; color: transparent;"/>
        </a>
     </div>
        <nav class="space-y-1 px-3 mt-4">
            <a class="group flex items-center rounded-md p-2 text-base font-medium css-1tq05rq hover:bg-btnPurple" href="{{ route('home') }}">
            ...
            <a class="group flex items-center rounded-md p-2 text-base font-medium css-1tq05rq hover:bg-btnPurple" href="{{ route('cari') }}">
                <svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" class="mr-2" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                    <desc></desc>
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
           ...
           <div class="dropdown">
  <a class="group flex items-center rounded-md p-2 w-90 text-base font-medium css-1tq05rq dropdown-toggle hover:bg-btnPurple"  id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="cursor:pointer;">
  ...
  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
    <a class="dropdown-item hover:bg-btnPurple" href="{{ route('hitungwr') }}">Winrate</a>
    <a class="dropdown-item hover:bg-btnPurple" href="{{ route('hitungpointmw') }}">Magic Wheel</a>
    <a class="dropdown-item hover:bg-btnPurple" href="{{ route('hitungpointzodiac') }}">Zodiac</a>
  </div>
</div>
           <div class="dropdown">
  <a class="group flex items-center rounded-md p-2 w-90 text-base font-medium css-1tq05rq dropdown-toggle hover:bg-btnPurple"  id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="cursor:pointer;">
  ...
  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
    ...
    <a class="dropdown-item hover:bg-btnPurple" href="{{ route('topten') }}">Top Ranking</a>
  </div>
</div>
</div>
